 ...
 */
export const BranchPaths = ({ branches, containerRef }: BranchPathsProps) => {
  ...
  useEffect(() => {
    ...
    const getDepth = (branchName: string) => {
      let depth = 0;
      let current = branches.find((b) => b.name === branchName);
      while (current?.parent) {
        depth++;
        current = branches.find((b) => b.name === current?.parent);
      }
      return depth;
    };
