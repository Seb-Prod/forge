...
/** Configuration des statuts Git affichés dans le filtre. */
...
export const GitStatusFilter = ({
  ...
  return (
    <Box surface="base" shadow="md" radius="md" border="xs">
      ...
      <Box>
        {STATUS_CONFIG.map(({ key, label, tone }) => {
          const paths = gitData[key as StatusKey] ?? [];
          const selectedCount = countSelected(paths);
          const isAllSelected  = paths.length > 0 && selectedCount === paths.length;
